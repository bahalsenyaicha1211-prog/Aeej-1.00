...
@section('title', 'Trésorerie • Nouvelle dépense')
@section('header', 'Enregistrer une dépense')

@section('content')
<div class="admDash">
    <div class="admDash__head">
        <div>
            <h1 class="admDash__title text-white">Nouvelle dépense</h1>
            <p class="admDash__sub">Ajoutez une ou plusieurs lignes (désignation + montant), le total est calculé automatiquement.</p>
        </div>
                ...
                <form method="POST" action="{{ route('tresorerie.depenses.store') }}" id="depense-form">
                    ...
                    <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 20px;">
                        <div class="field">
                            ...
                            <input class="input" name="nom_evenement" value="{{ old('nom_evenement') }}" required style="width:100%;" placeholder="ex: Journée culturelle AEEJ">
                        </div>
                        <div class="field">
                            <label class="admKpi__label text-white">Date de la dépense *</label>
                            <input class="input" type="date" name="date_depense" value="{{ old('date_depense', now()->toDateString()) }}" max="{{ now()->toDateString() }}" required style="width:100%;">
                        </div>
                    </div>
