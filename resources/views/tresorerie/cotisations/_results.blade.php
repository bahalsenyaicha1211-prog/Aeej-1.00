@if($tab === 'annuelle')
    <div class="card" style="margin-bottom:16px;">
        ...
        <form method="GET" data-live-search="#cotisations-results" action="{{ route('tresorerie.cotisations.index') }}" style="display:flex; gap:10px; align-items:center; flex-wrap:wrap; margin-bottom:14px;">
            <input type="hidden" name="tab" value="annuelle">
            <input class="input" type="text" name="q" value="{{ $q }}" placeholder="Rechercher par nom ou matricule..." style="max-width:260px;" autocomplete="off">
            ...
            @if(request('annee') || $q !== '')
                <a class="btn btn--ghost" data-live-search-link="#cotisations-results" href="{{ route('tresorerie.cotisations.index', ['tab' => 'annuelle']) }}">Réinitialiser</a>
            @endif
        </form>
...
@else
    <div class="card" style="margin-bottom:16px;">
        ...
        <form method="GET" data-live-search="#cotisations-results" action="{{ route('tresorerie.cotisations.index') }}" style="display:flex; gap:10px; align-items:center; flex-wrap:wrap; margin-bottom:14px;">
            ...
            <input class="input" type="text" name="q" value="{{ $q }}" placeholder="Rechercher par nom, matricule ou motif..." style="max-width:280px;" autocomplete="off">
            <input class="input" type="number" name="annee" value="{{ request('annee') }}" placeholder="Année" style="max-width:140px;">
            <button class="btn btn--ghost" type="submit">Filtrer</button>
            @if(request('annee') || $q !== '')
