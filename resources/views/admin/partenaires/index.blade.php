...
<div class="admDash">
    ...
    {{-- Filtre --}}
    <form method="GET" data-live-search="#partenaires-results" action="{{ route('admin.partenaires.index') }}" style="margin-bottom: 16px; display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
        <input class="input" type="text" name="q" value="{{ $q }}" placeholder="Rechercher un partenaire…" style="max-width:280px; width:100%;" autocomplete="off">
        ...
        @if($q !== '' || $slug !== '')
            <a class="admQuick__btn" data-live-search-link="#partenaires-results" href="{{ route('admin.partenaires.index') }}" style="text-decoration:none;">Réinitialiser</a>
        @endif
    </form>

    <div id="partenaires-results">
        @include('admin.partenaires._results')
    </div>
</div>
@endsection
